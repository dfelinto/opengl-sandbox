...
uniform vec3 unf19;
uniform float unf24;
const float cons26 = float(1.000000000000);
uniform float unf27;
uniform vec3 unf33;
const float cons43 = float(1.000000000000);
uniform vec4 unf44;
uniform vec3 unf46;
const float cons54 = float(1.000000000000);
uniform float unf57;
uniform float unf58;
uniform float unf59;
uniform float unf60;
uniform float unf61;
uniform vec4 unf65;
const float cons70 = float(1.000000000000);
const float cons73 = float(1.000000000000);
...
void main()
{
	...
	set_rgba_zero(tmp1);
	shade_norm(facingnormal, tmp3);
	shade_view(varposition, tmp5);
	set_rgb_zero(tmp6);
	set_rgb_zero(tmp7);
	lamp_visibility_other(varposition, unf9, tmp10, tmp11, tmp12);
	lamp_falloff_invsquare(unf13, tmp11, tmp15);
	lamp_visibility_clamp(tmp15, tmp17);
	shade_mul_value_v3(unf18, unf19, tmp20);
	shade_cooktorr_spec(tmp3, tmp10, tmp5, unf24, tmp25);
	shade_spec_t(cons26, unf27, tmp17, tmp25, tmp30);
	shade_add_spec(tmp30, tmp20, unf33, tmp34);
	shade_add_clamped(vec4(tmp7, 1.0), vec4(tmp34, 1.0), tmp37);
	shade_clamp_positive(tmp37, tmp39);
	shade_clamp_positive(vec4(tmp6, 1.0), tmp41);
	shade_maddf(tmp41, cons43, unf44, tmp45);
	shade_add_mirror(unf46, tmp1, tmp45.rgb, tmp49);
	shade_add(vec4(tmp49, 1.0), tmp39, tmp52);
	mtex_alpha_to_col(tmp52, cons54, tmp55);
	shade_mist_factor(varposition, unf57, unf58, unf59, unf60, unf61, tmp62);
	mix_blend(tmp62, tmp55, unf65, tmp66);
	shade_alpha_opaque(tmp66, tmp68);
	mtex_alpha_to_col(tmp68, cons70, tmp71);
	output_node(tmp71, cons73, tmp74);
	linearrgb_to_srgb(tmp74, tmp76);
